...
function filesToPhotos(fileList) {
    ...
    files.map(
      (file) =>
        new Promise((resolve) => {
          ...
          reader.onload = () => resolve({ src: reader.result, alt: file.name.replace(/\.[^.]+$/, "") });
...
export default function GalleryAdmin({ categories, photos }) {
  ...
  async function handleFiles(fileList) {
    ...
    const newPhotos = await filesToPhotos(fileList);
    if (newPhotos.length === 0) return;
    await addPhotos(active, newPhotos);
    router.refresh();
  }
